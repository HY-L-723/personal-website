...
5. 填写 `AUTH_ADMIN_EMAIL` 和初始化使用的 `AUTH_ADMIN_PASSWORD`（12–128 位）。初始化后清空后者，登录密码的哈希保存在数据库中。
   首次初始化时 `AUTH_ADMIN_USER_IDS` 必须留空，由初始化命令生成真实 ID；不要填写昵称、邮箱或自选编号。
...
本机 MySQL 联调已通过：表结构迁移及重复执行检查、网站注册与密码登录、站长与普通用户权限、会话续期 Cookie 转发、账号服务重启后身份保留、退出后旧会话持续失效、过期会话拒绝及数据库登录限流。验收只清理其生成的临时账号和关联数据，保留站长账号与配置。

可显式运行 `npm run test:auth:mysql` 复验。它要求网站已在本机运行、账号服务暂时停止，并在私密 `.env.auth` 中临时提供已存在站长的 `AUTH_ADMIN_PASSWORD`；脚本自行启动和重启账号服务，完成后停止测试服务，不会启动或停止 MySQL 本身。该命令会创建一个随机临时账号、定向调整它的会话期限并在结束时定向删除；不会作为普通单元测试自动运行。限流测试后，本机登录可能需要等待约一分钟再尝试；不会为测试清空限流表。复验后清空初始化密码字段，再运行 `npm run auth:dev`。

真实 Worker 联调修正了两个与 Node 单元测试不同的行为：采用手动处理并拒绝后端重定向，防止凭据被转发到其他目标；账号服务重启造成连接失效时，仅会话读取允许在同一超时窗口内重试一次，登录和注册等写请求绝不自动重放。
